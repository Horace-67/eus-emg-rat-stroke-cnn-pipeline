...
- `plot_high_low_ratio_ratlevel.m`  
  補充分析程式，用於計算傳統高低頻能量比（100 ~ 300 Hz / 50 ~ 100 Hz）之大鼠層級基準特徵，並輸出群組比較圖、個體分布圖。此程式對應成果報告中「基準比較：傳統頻帶能量比」之方法與結果。
- `gradcam_examples.m`  
  補充分析程式，用於讀取已儲存之 Grad-CAM 分析封包（pack），輸出代表性高活動與低活動視窗的 1D、2D Grad-CAM 視覺化結果，包含原始波形、Grad-CAM 熱圖、STFT 時頻圖及疊合圖。此程式主要對應成果報告中「模型可解釋性Grad-CAM分析與代表視窗選取」之方法與結果。
- `eus_rf_lopo_burst_multiwindow.m`  
  補充分析程式，用於比較不同固定視窗長度下之傳統特徵分類表現，並以隨機森林（Random Forest）作為統一基準分類器進行留一個體驗證。此程式主要對應成果報告中「視窗長度比較與主要設定」之方法與結果，主要用於視窗長度選擇，作為後續 1D / 2D CNN 正式分析之設定依據，而非本研究最終模型。
---
...
公開版程式目前會從 `.mat` 檔中讀取 EUS-EMG 訊號。在原始資料格式中，EUS-EMG 對應為第 2 個 channel；雖原始記錄同時包含膀胱壓訊號，但本流程中並未使用膀胱壓進行模型分析。
...
本儲存庫另附 `label_example.xlsx` 作為標註檔格式示意，內容僅為節錄範例，非完整研究資料。

---

## 六、輸出結果說明

程式 eus_run_main_public.m 執行後，會於 `results/` 資料夾中輸出結果，內容可能包括：
...
- `plot_high_low_ratio_ratlevel.m`：對應傳統頻帶能量比基準比較
- `gradcam_examples.m`：對應代表性 Grad-CAM 視覺化輸出
- `eus_rf_loso_burst_multiwindow.m`：對應視窗長度比較與主要設定選擇
